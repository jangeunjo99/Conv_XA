...
class Dataset(object):
    ...
    def __init__(self, config):
        self.config = config
        self.dataset_name = config['dataset']  # 데이터셋 이름 설정
        self.logger = getLogger()
        self._from_scratch()  # 데이터를 처음부터 로드
    ...
    def _from_scratch(self):  
        ...
        self._get_preset()  # 데이터셋의 설정
        self._get_field_from_config()  # 데이터셋의 필드 정보
        self._load_data(self.dataset_name, self.dataset_path)  # 데이터 로드
        self._init_alias()  # 별칭 초기화
        self._data_processing()  # 데이터 전처리 

    def _get_preset(self):
        """Initialization useful inside attributes.
        클래스 내부에서 유용한 속성들 초기화 
        """
    ...
    def _get_field_from_config(self):
        ...
        설정 파일에서 공통 필드 이름 초기화
        """
        self.uid_field = self.config['USER_ID_FIELD']  # 사용자 ID 필드
        self.iid_field = self.config['ITEM_ID_FIELD'] # 아이템 ID 필드
        self.label_field = self.config['LABEL_FIELD'] # 라벨 필드
        self.time_field = self.config['TIME_FIELD'] # 시간 필드
        self.title_field = self.config['ITEM_ID_NAME_FIELD']
    ...
    def _data_processing(self):
        ...
        """
        self.feat_name_list = self._build_feat_name_list()  # 특정 이름 리스트 생성 
        if self.benchmark_filename_list is None:
            self._data_filtering()

        self._remap_ID_all()  # 모든 ID 재매핑
        self._user_item_feat_preparation() # 사용자와 아이템의 특징 준비
        self._fill_nan() # 누락된 값 대체
        self._set_label_by_threshold() # 임계값에 따라 라벨 설정
        self._normalize() # 정규화
        self._preload_weight_matrix() # 가중치 행렬 미리 로드

    def _data_filtering(self):  
        # 데이터셋에서 불필요하거나 유효하지 않은 데이터 제거하는 여러 단계 포함 
        """Data filtering
        ...
        """
        self._filter_nan_user_or_item()  # 사용자 ID 또는 아이템 ID 누락된 데이터 삭제
        self._remove_duplication()  # 동일한 사용자와 아이템 간의 중복된 상호작용 데이터 제거
        self._filter_by_field_value() # 특정 필드 값에 따라 데이터 필터링
        self._filter_inter_by_user_or_item() # 특정 사용자나 아이템과 관련된 상호작용 데이터 제거
        self._filter_by_inter_num() # 각 사용자와 아이템이 최소한 K개의 상호작용 가지도록 필터링
        self._reset_index() # 필터링 후, 데이터프레임의 인덱스가 연속적이지 않게 되므로 인덱스 리셋

    def _build_feat_name_list(self):
        # 데이터셋에 로드된 특징들의 이름 리스트를 구성 
        """Feat list building.
    ...
    def _load_data(self, token, dataset_path):
        ...
        if not os.path.exists(dataset_path):
            self._download()
        self._load_inter_feat(token, dataset_path)   # 상호작용 특징 로드
        self.user_feat = self._load_user_or_item_feat(token, dataset_path, FeatureSource.USER, 'uid_field') # 사용자 및 아이템 특징 각각 로드
    ...
    def _filter_by_inter_num(self):
        ...
        # 필터링 전 유저 명수 출력
        initial_user_count = len(self.inter_feat[self.uid_field].unique())
        print(f"Number of users before filtering: {initial_user_count}")
        ...
        # 필터링 후 유저 명수 출력
        final_user_count = len(self.inter_feat[self.uid_field].unique())
        print(f"Number of users after filtering: {final_user_count}")
        ...
        # 유저별 시퀀스 길이 계산
        user_interactions = self.inter_feat.groupby(self.uid_field).size()
        
        # 노이즈를 추가할 유저 수 계산
        num_noisy_users = int(len(user_interactions) * user_ratio)
        
        # 노이즈를 추가할 유저 랜덤 선택
        noisy_users = np.random.choice(user_interactions.index, num_noisy_users, replace=False)
        
        all_item_ids = self.inter_feat[self.iid_field].unique()
        
        # 유저별로 노이즈 추가
        for user in noisy_users:
            user_indices = self.inter_feat[self.inter_feat[self.uid_field] == user].index
            seq_length = len(user_indices)
            num_noisy_items = int(round(seq_length * noise_ratio))
            
            if num_noisy_items > 0:
                noisy_indices = np.random.choice(user_indices, num_noisy_items, replace=False)
                noisy_item_ids = np.random.choice(all_item_ids, num_noisy_items, replace=True)
                self.inter_feat.loc[noisy_indices, self.iid_field] = noisy_item_ids
