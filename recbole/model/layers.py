...
class CATMultiHeadAttention(nn.Module):
    def __init__(self, n_heads, hidden_size, attribute_hidden_size, feat_num, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps, fusion_type, max_len):
        ...
        # Validate hidden size and number of attention heads
        if hidden_size % n_heads != 0:
            raise ValueError(f"The hidden size ({hidden_size}) is not a multiple of the number of attention heads ({n_heads})")

        self.num_attention_heads = n_heads
        self.attention_head_size = hidden_size // n_heads
        self.all_head_size = self.num_attention_heads * self.attention_head_size
        self.attribute_attention_head_size = hidden_size // n_heads
        self.attribute_all_head_size = self.num_attention_heads * self.attention_head_size
        self.fusion_type = fusion_type
        self.max_len = max_len

        # Linear layers for item attention
        self.item_query = nn.Linear(hidden_size, self.all_head_size)
        self.item_key = nn.Linear(hidden_size, self.all_head_size)
        self.item_value = nn.Linear(hidden_size, self.all_head_size)

        # Linear layers for category attention
        self.cate_value = nn.Linear(hidden_size, self.all_head_size)
        self.cate_query = nn.Linear(attribute_hidden_size[0], self.all_head_size)

        # Fusion layer
        if self.fusion_type == 'concat':
            self.fusion_layer = nn.Linear(self.max_len * (2 + feat_num), self.max_len)
        elif self.fusion_type == 'gate':
            self.fusion_layer = VanillaAttention(self.max_len, self.max_len)
    ...
    def transpose_for_scores_attribute(self, x):
        new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attribute_attention_head_size)
        x = x.view(*new_x_shape)
        return x.permute(0, 2, 1, 3)

    def forward(self, input_tensor, cate_tensor):
        # Process item attention
        item_query_layer = self.transpose_for_scores(self.item_query(input_tensor))
        item_key_layer = self.transpose_for_scores(self.item_key(input_tensor))
        item_value_layer = self.transpose_for_scores(self.item_value(input_tensor))

        item_attention_scores = torch.matmul(item_query_layer, item_key_layer.transpose(-1, -2))
        item_attention_scores = item_attention_scores / math.sqrt(self.attention_head_size)
        item_attention_probs = nn.Softmax(dim=-1)(item_attention_scores)
        item_attention_probs = self.attn_dropout(item_attention_probs)

        item_context_layer = torch.matmul(item_attention_probs, item_value_layer)
        item_context_layer = item_context_layer.permute(0, 2, 1, 3).contiguous()
        item_new_context_layer_shape = item_context_layer.size()[:-2] + (self.all_head_size,)
        item_context_layer = item_context_layer.view(*item_new_context_layer_shape)
        item_hidden_states = self.dense(item_context_layer)
        item_hidden_states = self.out_dropout(item_hidden_states)
        item_hidden_states = self.LayerNorm(item_hidden_states + input_tensor)

        # Process category attention
        cate_tensor = cate_tensor.squeeze(2)
        cate_query_layer = self.transpose_for_scores_attribute(self.cate_query(cate_tensor))
        cate_value_layer = self.transpose_for_scores_attribute(self.cate_value(cate_tensor))

        cate_attention_scores = torch.matmul(item_query_layer, cate_query_layer.transpose(-1, -2))
        cate_attention_scores = cate_attention_scores / math.sqrt(self.attribute_attention_head_size)
        cate_attention_probs = nn.Softmax(dim=-1)(cate_attention_scores)
        cate_attention_probs = self.attn_dropout(cate_attention_probs)

        cate_context_layer = torch.matmul(cate_attention_probs, cate_value_layer)
        cate_context_layer = cate_context_layer.permute(0, 2, 1, 3).contiguous()
        cate_new_context_layer_shape = cate_context_layer.size()[:-2] + (self.attribute_all_head_size,)
        cate_context_layer = cate_context_layer.view(*cate_new_context_layer_shape)
        cate_hidden_states = self.dense(cate_context_layer)
        cate_hidden_states = self.out_dropout(cate_hidden_states)
        cate_hidden_states = self.LayerNorm(cate_hidden_states + cate_tensor)

        return item_hidden_states, cate_hidden_states
    
class CATTransformerLayer(nn.Module):
    def __init__(
        self, n_heads, hidden_size, attribute_hidden_size, feat_num, intermediate_size, hidden_dropout_prob, attn_dropout_prob, hidden_act,
        layer_norm_eps, fusion_type, max_len
    ):
        super(CATTransformerLayer, self).__init__()
        
        # Multi-head attention mechanism
        self.multi_head_attention = CATMultiHeadAttention(
            n_heads, hidden_size, attribute_hidden_size, feat_num, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps, fusion_type, max_len,
        )
    ...
    def forward(self, item_hidden_states, cate_hidden_states):
        # Apply multi-head attention
        item_attention_output, cate_attention_output = self.multi_head_attention(item_hidden_states, cate_hidden_states)
        
        # Apply feed-forward network
        item_feedforward_output = self.feed_forward(item_attention_output)
        cate_feedforward_output = self.feed_forward(cate_attention_output)
        
        return item_feedforward_output, cate_feedforward_output
...
class CATTransformerEncoder(nn.Module):
    ...
    def forward(self, item_hidden_states, cate_hidden_states, output_all_encoded_layers=True):
        all_encoder_layers = []

        # Process each layer
        for layer_module in self.layer:
            item_hidden_states, cate_hidden_states = layer_module(item_hidden_states, cate_hidden_states)
            if output_all_encoded_layers:
                all_encoder_layers.append((item_hidden_states, cate_hidden_states))
...
class FeatureSeqEmbLayer(ContextSeqEmbAbstractLayer):
    ...
    def __init__(self, dataset, embedding_size, selected_features, pooling_mode, device):
        ...
        self.field_names = {'item': selected_features}  # 선택된 특성 item 키와 함께 사전에 저장
