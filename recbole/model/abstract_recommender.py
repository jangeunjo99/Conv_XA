...
class SequentialRecommender(AbstractRecommender):
    ...
    def gather_indexes(self, output, gather_index):
        ...
        if(gather_index >= output.shape[1]).any():
            max_index = torch.tensor(output.shape[1] - 1, dtype=torch.long, device=gather_index.device)
            gather_index = torch.minimum(gather_index, max_index)
